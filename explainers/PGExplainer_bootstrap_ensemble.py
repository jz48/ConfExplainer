...
class PGExplainer_bootstrap_ensemble(BaseExplainer):
    """
    ...
    def train(self, indices=None):
        ...
        for i in range(self.n_models):
            n_indices.append(np.random.choice(indices, size=(len(indices)-int(len(indices)/self.n_models)), replace=False))

        print('random samples for deep ensemble:', n_indices)
        for i in range(self.n_models):
            self.explainer_model = nn.Sequential(
                nn.Linear(self.expl_embedding, 64),
                nn.ReLU(),
            ...
            self.train_1_model(indices=n_indices[i])
            self.expl_models.append(self.explainer_model)


    def train_1_model(self, indices=None):
        """
