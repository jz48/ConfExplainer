...
class MainExplainer:
    def __init__(self, dataset_name, model_name, explainer_name, wandb_log=False, loss_type='ib',
        ...
        if dataset_name in ['bareg1', 'bareg2', 'bareg3', 'crippen', 'bareg3', 'triangles', 'triangles_small']:
            self.type = 'reg'
        else:
            self.type = 'cls'
    ...
    def explain(self):
        if self.dataset_name in ['crippen', 'bareg3']:
        ...
        for turn_, s in enumerate(self.seeds):
            ...
            fid_plus, fid_minus = self.evaluate_robust_fid(explanations)
            print("auc score:", auc_score, "rdd score: ", rdd_score, "fid_plus: ", fid_plus, "fid_minus: ", fid_minus)

            scores.append([s, auc_score, rdd_score, fid_plus, fid_minus])
    ...
    def evaluate_robust_fid(self, explanations):
        sum_fid_plus = 0.0
        sum_fid_minus = 0.0
        for idx, graph_, expl_ in explanations:
            graph = graph_.detach()
            expl = expl_.detach()
            feat = self.explainer_manager.explainer.features[idx]
            # ground_truth = self.dataset_loader.edge_ground_truth[idx]
            mask_pred_plus = self.explainer_manager.explainer.model_to_explain(feat, graph, edge_weights=(1 - expl))
            mask_pred_minus = self.explainer_manager.explainer.model_to_explain(feat, graph, edge_weights=expl)
            fid_plus = self.dataset_loader.labels[idx][0] - mask_pred_plus.detach().numpy()[0][0]
            fid_minus = self.dataset_loader.labels[idx][0] - mask_pred_minus.detach().numpy()[0][0]
            sum_fid_plus += fid_plus
            sum_fid_minus += fid_minus
        return sum_fid_plus / len(explanations), sum_fid_minus / len(explanations)
    ...
    def show_results(self):
        ...
        rdd = np.mean(rdd_scores)
        rdd_std = np.std(rdd_scores)
        fid_plus = np.mean([i[3] for i in scores])
        fid_plus_std = np.std([i[3] for i in scores])
        fid_minus = np.mean([i[4] for i in scores])
        fid_minus_std = np.std([i[4] for i in scores])

        fid_delta = fid_plus - fid_minus
        print(self.explainer_name, self.dataset_name, self.model_name)
        print("Auc score: %.4f (%.4f)" % (auc, auc_std))
        print("RDD score: %.4f (%.4f)" % (rdd, rdd_std))
        print("FID plus: %.4f (%.4f)" % (fid_plus, fid_plus_std))
        print("FID minus: %.4f (%.4f)" % (fid_minus, fid_minus_std))
        print("FID delta: %.4f" % fid_delta)

        reversed_auc_scores = [1 - i if i <= 0.5 else i for i in auc_scores]
        reversed_auc = np.mean(reversed_auc_scores)
        reversed_auc_std = np.std(reversed_auc_scores)
        print("Reversed Auc score: %.4f (%.4f)" % (reversed_auc, reversed_auc_std))
        # return auc, auc_std, rdd, rdd_std
        return {'auc': auc, 'auc_std': auc_std, 'rdd': rdd, 'rdd_std': rdd_std,
                'reversed_auc': reversed_auc, 'reversed_auc_std': reversed_auc_std}
