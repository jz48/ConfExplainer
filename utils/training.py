...
class GNNTrainer:
    def __init__(self, dataset_name, model_name, lr=0.001, epochs=3000, clip_max=2.0, batch_size=64, early_stopping=1000,
    ...
    def train(self):
        if self.task == 'node':
            ...
            if self.type == 'reg':
                return self.train_graph_reg()
            elif self.type == 'cls':
                return self.train_graph_cls()
            else:
                assert 0
    ...
    def train_graph_cls(self):
        ...
        for epoch in range(0, self.epochs):
            ...
            with torch.no_grad():
                ...
                for data in train_loader:
                    ...
                    count += len(data.y)
                train_acc = int(train_sum) / count
                train_loss = float(loss) / count
        ...
        print(f"final train_acc:{train_acc}, val_acc: {val_acc}, test_acc: {test_acc}")
        self.model_manager.store_best_checkpoint(self.model_name, self.dataset_name, model, train_acc, val_acc, best_epoch)
        self.model_manager.store_checkpoint(self.model_name, self.dataset_name, model, train_acc, val_acc, -2)
        # store_checkpoint(_paper, _dataset, model, train_acc, val_acc, test_acc)

    def train_graph_reg(self):
        model = self.model_manager.model
        ...
        for epoch in range(0, self.epochs):
            ...
            if self.type == 'reg':
                with torch.no_grad():
                    ...
                    train_rmse = math.sqrt(float(train_mse) / int(count))
                    train_mape = float(train_mape) / int(count)

                    def do_val_alt(the_loader):
                        the_mse, the_mape = 0.0, 0.0
                        count = 0
                        for data in the_loader:
                            out = model(data.x, data.edge_index, data.batch, data.edge_mask)
                            ...
                            count += len(data.y)
                        # print('val mape: ', val_mape, len(val_set_alt))
                        the_rmse = math.sqrt(the_mse / count)
                        the_mape = float(the_mape) / count
                        return the_rmse, the_mape

                    val_rmse, val_mape = do_val_alt(val_loader)
        ...
        self.model_manager.load_best_model(best_epoch, self.eval_enabled)
        model = self.model_manager.model
        self.model_manager.store_best_checkpoint(self.model_name, self.dataset_name, model, train_mape, val_mape, best_epoch)
        self.model_manager.store_checkpoint(self.model_name, self.dataset_name, model, train_mape, val_mape, -2)
